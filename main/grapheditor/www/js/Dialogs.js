...
var OpenDialog = function()
{
	...
	//main_el.classList.add("st_open_graph");
	...
	// iframe.classList.add("st_open_graph");

	//this.container = main_el;
...
var FilenameDialog = function(editorUi, filename, buttonText, fn, label, validateFn, content, helpLink, closeOnBtn, cancelFn, hints, w)
{
	...
	//table.after('<div class="stockwell_b24">werwerwer</div>')
...
var ExportDialog = function(editorUi)
{
	...
	//stockwell
	table.classList.add('stockwell_import')
	...
	td.appendChild(div_t);
	div_t.classList.add('stockwell_import_div_title')
	div_t.innerHTML = 'Сохранить как:'
	row.appendChild(td);
	tbody.appendChild(row);
	...
	td.appendChild(div_l);
	div_l.classList.add('stockwell_import_div')
	...
	var user_id = 0;
	div_l.appendChild(div_user);
	div_user.classList.add('stockwell_import_div_user')
	div_user.setAttribute('data-st-import', 'user')
	div_user.innerHTML = 'Отправить пользователю'
	mxEvent.addListener(div_user, 'click', function () {

		BX24.selectUser(function (user) {
			div_user.innerHTML = user.name;
			div_user.setAttribute('data-st-user-id', user.id)
		})


		$('.stockwell_import_div div').each(function () {
			$(this).removeClass('active');
		})

		$('.stockwell_import_div_user').addClass('active');
	});


	//disk
	var div_disk = document.createElement("div");
	div_l.appendChild(div_disk);
	div_disk.classList.add('stockwell_import_div_disk')
	div_disk.setAttribute('data-st-import', 'disk')
	div_disk.innerHTML = 'Сохранить на Битрикс24 диск'
	mxEvent.addListener(div_disk, 'click', function () {
		$('.stockwell_import_div div').each(function () {
			$(this).removeClass('active');
		})

		$('.stockwell_import_div_disk').addClass('active');
	});

	//life
	var div_life = document.createElement("div");
	div_l.appendChild(div_life);
	div_life.classList.add('stockwell_import_div_life')
	div_life.setAttribute('data-st-import', 'life')
	div_life.innerHTML = 'Отправить в живую ленту'
	mxEvent.addListener(div_life, 'click', function () {
		$('.stockwell_import_div div').each(function () {
			$(this).removeClass('active');
		})

		$('.stockwell_import_div_life').addClass('active');
	});

	//save
	var div_save = document.createElement("div");
	div_l.appendChild(div_save);
	div_save.classList.add('stockwell_import_div_save')
	div_save.setAttribute('data-st-import', 'save')
	div_save.classList.add('active')
	div_save.innerHTML = 'Сохранить на компьютер'

	mxEvent.addListener(div_save, 'click', function () {
		$('.stockwell_import_div div').each(function () {
			$(this).removeClass('active');
		})

		$('.stockwell_import_div_save').addClass('active');
	});
	...
	var saveBtn = mxUtils.button('Применить', mxUtils.bind(this, function()
	{
		...
		$('.stockwell_import_div div').each(function () {
			if ($(this).hasClass('active')) {
				type_export = $(this).attr('data-st-import')
				if(type_export == 'user') {
					user_id = $(this).attr('data-st-user-id')
				}
			}
		})
		...
		else
		{
	    	...
	    	var s = Math.max(0, parseFloat(zoomInput.value) || 100) / 100;
			var b = 0;//Math.max(0, parseInt(borderInput.value));
			...
			ExportDialog.exportFile(editorUi, name, format, bg, s, b, dpi, type_export, user_id);
		}
	}));
 ...
 */
ExportDialog.exportFile = function(editorUi, name, format, bg, s, b, dpi, type_export, user_id)
{
	var graph = editorUi.editor.graph;
    ...
    else
    {
		...
		if (param.length <= MAX_REQUEST_SIZE && w * h < MAX_AREA)
		{
			...
			// 	req.send();
				...
				var svg = graph.getSvg(bg, s, b);


				generatePng(format, encodeURIComponent(name), w, h, svg, type_export, user_id);
			...
			// var str = s.serializeToString(svg);
				...
				//console.log(localStorage[encodeURIComponent(name)]);
